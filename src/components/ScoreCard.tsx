...
export const ScoreCard: React.FC<ScoreCardProps> = ({ scores, onPlayAgain }) => {
  ...
  const renderScoreSection = (holes: HoleScore[], startIndex: number) => {
    const sectionStrokes = holes.reduce((sum, hole) => sum + hole.strokes, 0);
    const sectionPar = holes.reduce((sum, hole) => sum + hole.par, 0);
    const sectionRelativeToPar = sectionStrokes - sectionPar;
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={{
                padding: '8px',
                fontWeight: 'bold',
                color: sectionRelativeToPar > 0 ? '#ff4444' : sectionRelativeToPar < 0 ? '#44aa44' : '#000000'
              }}>
                {sectionRelativeToPar === 0 ? 'E' : sectionRelativeToPar > 0 ? `+${sectionRelativeToPar}` : sectionRelativeToPar}
              </td>
            </tr>
          </tbody>
        </table>
